...
import (
	...
	"example.com/proj/pkg/dto"
	"example.com/proj/pkg/mem"
)
...
func main() {
	...
	http.HandleFunc("/isusing", isUsingHandler)
...
func isSemaphoreInUse() bool {
	select {
	case semaphore <- struct{}{}: // Try to add to the semaphore
		<-semaphore  // If successful, remove it again
...
func isUsingHandler(w http.ResponseWriter, r *http.Request) {
	isInUse := isSemaphoreInUse()
	fmt.Println("Is semaphore in use: ", isInUse)
	if isInUse {
		w.WriteHeader(http.StatusServiceUnavailable)
		return
	}
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(isInUse)
}
...
func handleMultipleCodes(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	resBody := dto.ResponseBody{Results: results}
